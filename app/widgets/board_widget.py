...
class BoardWidget(CardWidget):
    ...
    def drawActionPreview(self, painter: QPainter):
        ...
        destination = (self.board.player_pos[1 - int(self.board.state[4, 0, 0])][0] + move[0],
                       self.board.player_pos[1 - int(self.board.state[4, 0, 0])][1] + move[1])

        active_player_color = BLUE if self.board.state[4, 0, 0] == 1 else GREEN
    ...
    def drawAvailablePositions(self, painter):
        ...
        available_positions = []
        light_color = BLUE if self.board.state[4, 0, 0] == 1 else GREEN

        for action in self.board.available_actions:
            pos = (
                self.board.action_to_pos[action // 4][0] +
                self.board.player_pos[int(1 - self.board.state[4, 0, 0])][0],
                self.board.action_to_pos[action // 4][1] +
                self.board.player_pos[int(1 - self.board.state[4, 0, 0])][1]
            )
    ...
    def drawWall(self, painter: QPainter):
        ...
        for i in range(self.board_len):
            for j in range(self.board_len):
                if self.board.state[2, i, j] == 1:
                    self.drawHorizontalWall(painter, color, i, j)
                if self.board.state[3, i, j] == 1:
    ...
    def mouse_pos_to_action(self, mouse_pos_x, mouse_pos_y):
        ...
        active_pos = self.board.player_pos[int(1 - self.board.state[4, 0, 0])]
    ...
    def doAction(self, action, change_history=True):
        ...
        self.active_player_pos_index = 0 if self.board.state[4, 0, 0] == 1 else 1
